...
class CompelForSDXL:
    ...
    def __call__(self,
        ...
        if embeds_left.shape[1] > embeds_right.shape[1]:
            padding = self.compel_2("")
            while embeds_left.shape[1] > embeds_right.shape[1]:
...
def _make_compel_input_with_optional_negative(main_prompt, negative_prompt):
    if negative_prompt is None:
        return main_prompt, None

    if type(negative_prompt) is str:
        negative_prompt = [negative_prompt]
    if type(main_prompt) is str:
        main_prompt = [main_prompt]
    main_input = main_prompt + negative_prompt
    negative_main_start_index = len(main_prompt)
